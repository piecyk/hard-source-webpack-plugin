        ...
        md5CacheSerializer
          ...
          .then(_md5Cache => {
            Object.keys(_md5Cache).forEach(key => {
              ...
              cachedMd5s[key] = _md5Cache[key]
                ? _md5Cache[key].hash
                : '';

              if (!_md5Cache[key]) {
                console.log('No cached value for key =', key);
              }
            });
            md5Cache = _md5Cache;
          })
          .then(() => {
            ...
            fileDependencies = dependencies.filter(
              file => md5Cache[file] && md5Cache[file].isFile,
            );
            contextDependencies = dependencies.filter(
              file => md5Cache[file] && md5Cache[file].isDirectory,
            );
          }),
    );
